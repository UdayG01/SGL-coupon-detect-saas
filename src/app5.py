...
class user_app_callback_class(app_callback_class):
    def __init__(self, sku_code):
        ...
        # CSV log
        self.csv_filename = f"detection_log_{datetime.now().strftime('%Y%m%d')}.csv"
        self.init_csv()

        # API
        self.api_url = "http://localhost:1880/result"

        self.sender_email = "uday.renataiot@gmail.com"
        self.receiver_email = "uday.gupta@renataiot.com"
        self.app_password = "gwpc fwfi lnrw pphm"
    ...
    def process_frame(self, buckets, coupons):
        ...
        if self.coupon_cross_frames >= self.cross_threshold:
            ...
            self.reset_counters()
            self.send_email_text(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "NG", receiver_email="uday.gupta@renataiot.com")
