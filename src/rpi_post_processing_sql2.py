...
class user_app_callback_class(app_callback_class):
    def __init__(self):
        ...
        with open(self.results_file, mode="w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["SNo", "timestamp", "result"])
            writer.writeheader()
    ...
    def log_result(self, result):
        ...
        row = {"SNo": self.serial_no, "timestamp": timestamp, "result": result}
...
def app_callback(pad, info, user_data):
    ...
    if user_data.use_frame and frame is not None:
        ...
        panel_width, panel_height = 250, 100
        x1, y1 = width - panel_width - 10, 10
        x2, y2 = width - 10, 10 + panel_height
        ...
        # Display OK/NG signal
        if result == "OK":
            color = (0, 255, 0)  # Green
            cv2.putText(frame, "Signal: OK", (x1 + 10, y1 + 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        elif result == "NG":
            color = (0, 0, 255)  # Red
            cv2.putText(frame, "Signal: NG", (x1 + 10, y1 + 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        else:
            cv2.putText(frame, "Signal: --", (x1 + 10, y1 + 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)

        # Display counter below signal
        cv2.putText(frame, f"Counter: {user_data.counter}", (x1 + 10, y1 + 70),
